 ...
 *Must called after pm_ppl_bucket_init
 * */
void
...
/*
 * Add key to pline's hashtable if it hasn't exit yet
 * The caller reponse for holding the pline->lock
 * @param[in] pop
 * @param[in] ppl
 * @param[in] pline
 * @param[in] key
 * @return pointer to the found log_block or the new added one
 * */
PMEM_PAGE_LOG_BLOCK*
 ...
 * old version used HASH_TABLE from InnoDB
 * */
plog_hash_t*
pm_ppl_hash_check_and_add_old(
		PMEMobjpool*				pop,
		PMEM_PAGE_PART_LOG*			ppl,
		PMEM_PAGE_LOG_HASHED_LINE*	pline,
		uint64_t					key	)
...
//	plog_hash_t* item;
//
//	item = pm_ppl_hash_get_old(pop, ppl, pline, key);
//	i_bit = -2; //init value 
//
//	if (item == NULL){
			...
			uint32_t			rec_size)
{
	...
	if ( (*log_src) == MLOG_TABLE_DYNAMIC_META) {
		 ...
		 * Now we use a special partition log for metadata
		 * */
		is_dict = true;
	...
	/*New in MySQL 8.0,
	 * Metadata: use the last bucket
	 * Otherwise: hash by key
	 * */	
	if (is_dict) {
		hashed = n - 1;
	} else {
		//PMEM_LOG_HASH_KEY(hashed, key, n);
		PMEM_LOG_HASH_KEY(hashed, key, (n - 1));
		assert(hashed < (n - 1));
	}

	//printf("BEGIN write rec on pline %zu key %zu\n", hashed, key);
retry:
		...
		plogbuf->state == PMEM_LOG_BUF_IN_FLUSH)
	{
		...
		//printf("WAIT_FOR_FLUSH write rec on pline %zu key %zu\n", hashed, key);
...
get_free_buf:
		...
		if (pfreepool->cur_free_bufs == 0 || 
			...
			//printf("WAIT_FOR_FREE_POOL write rec on pline %zu key %zu\n", hashed, key);
		...
		//pmemobj_rwlock_unlock(pop, &pline->lock);
		//printf("END (full) write rec on pline %zu key %zu\n", hashed, key);
		return rec_lsn;
		...
		//pmemobj_rwlock_unlock(pop, &pline->lock);
		//printf("END write rec on pline %zu key %zu\n", hashed, key);
		return rec_lsn;
 ...
 * Later, the master thread (MySQL 5.7) or checkpointer thread (MySQL 8.0) (1s interval) will call checkpoint based on this value 
			...
			)
{
	...
	if ( age > PMEM_CKPT_MAX_OFFSET) {
		/*The logic of is_req_checkpoint flag:
		 * 1) when this flag is on, trx threads will not call this function during writing REDO log 
		 * 2) set this flag to false only the oldest page managed by this pline is flush to disk, see pm_ppl_flush_page()*/
		...
		pmemobj_rwlock_wrlock(pop, &ppl->ckpt_lock);
		if (ppl->max_req_ckpt_lsn < pline->ckpt_lsn){
			ppl->max_req_ckpt_lsn = pline->ckpt_lsn;
		}
