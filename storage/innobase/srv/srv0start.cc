...
#endif // UNIV_PMEMOBJ_PART_PL and other methods
...
/*tdnguyen: Printout info about current method (right before srv_boot() called*/
#if defined (UNIV_AIO_IMPROVE)
  ib::info() << "n_slots_per_seg=" << srv_aio_n_slots_per_seg << ";\n";
#endif

/*Below methods shared the same code for allocate the global NVM pool manager*/
#if defined (UNIV_PMEMOBJ_LOG) || defined(UNIV_PMEMOBJ_DBW) || defined (UNIV_PMEMOBJ_BUF) || defined (UNIV_PMEMOBJ_WAL) || defined (UNIV_PMEMOBJ_PART_PL)

#if defined (UNIV_PMEMOBJ_PART_PL)
  ib::info() << "===== PL-NVM: Partition Logging in NVM =====\n";
#endif // UNIV_PMEMOBJ_PART_PL
	/*Init part 1 - before srv_boot()*/
...
#if defined (UNIV_PMEMOBJ_PART_PL)
  /*Init part 2 - after fsp_init() and before recv_sys_create()*/
...
#if defined (UNIV_PMEMOBJ_PART_PL)
	/*Init part 3 - Create necessary threads (after creating other threads*/

	printf("PMEM_INFO: ========>   create %zu flusher worker threads for PART-LOG\n", srv_ppl_n_log_flush_threads);
	/*create log flusher workers. The key is used to registered with performance schema*/
	for (i = 0; i < srv_ppl_n_log_flush_threads; ++i) {
		//os_thread_create(pm_log_flusher_worker, NULL, NULL); //MySQL 5.7
		os_thread_create(pm_log_flusher_thread_key, pm_log_flusher_worker); //MySQL 8.0
	}
...
#if defined (UNIV_PMEMOBJ_PART_PL)
	/* Init part 4 - We create or open PMEM part log files here */
...
files_checked:
  ...
  if (create_new_db) {
	...
	if (gb_pmw->ppl->is_new){
		/*If this is the first time we use the PL-NVM
		 * then use the InnoDB recovery mechanism
		 * */
...
#if defined (UNIV_PMEMOBJ_PART_PL)
	/*Im MySQL 8.0, this call may need needed*/
...
void srv_shutdown() {
  ...
  /*We close PL-NVM before InnoDB*/
  pm_wrapper_free(gb_pmw);

#endif //UNIV_PMEMBOJ_PART_PL and other shared methods
